...
describe("indexer", () => {
  ...
  it("indexes custom tool traces appended to a Codex session", async () => {
    ...
    try {
      ...
      expect(warm.indexed).toBe(1);
      expect(await store.getTraceEvents("codex:codex-custom-tail")).toEqual([
  ...
  it("reconciles persisted Code Mode tool state when runtime evidence arrives later", async () => {
    const store = createInMemoryStore();
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "agent-recall-v2-codex-tool-state-tail-"));
    try {
      ...
      const nestedCommands = (await store.getTraceEvents("codex:codex-tool-state-tail")).filter((event) => {
  ...
  it("restores an active Codex turn when lifecycle records arrive in separate scans", async () => {
    ...
    try {
      ...
      expect((await store.getCodexIncrementalState("codex:codex-lifecycle-tail")).messageProvenance).toContainEqual({
        messageIndex: 1,
      ...
      expect((await store.getAllMessages("codex:codex-lifecycle-tail")).some(
        (message) => message.sourceTurnId === "turn-rolled",
      )).toBe(false);
      expect((await store.getTraceEvents("codex:codex-lifecycle-tail")).some(
        (event) => event.sourceTurnId === "turn-rolled",
      )).toBe(false);
      expect((await store.getTokenEvents("codex:codex-lifecycle-tail")).map(
        (event) => event.sourceTurnId,
      )).toEqual(["turn-tail", "turn-rolled"]);
      expect((await store.getSession("codex:codex-lifecycle-tail"))?.tokenUsage?.totalTokens).toBe(33);
      expect((await store.listSessionTurns("codex:codex-lifecycle-tail")).find(
        (turn) => turn.sourceTurnId === "turn-tail",
      )?.totalTokens).toBe(11);
      expect((await store.getCodexIncrementalState("codex:codex-lifecycle-tail")).activeTurnIds).toEqual([]);
    } finally {
      await store.close();
      fs.rmSync(homeDir, { recursive: true, force: true });
    }
  ...
  it("deduplicates Codex token events when an appended tail repeats prior usage", async () => {
    ...
    try {
      ...
      expect(await store.getTokenEvents("codex:codex-token-tail")).toHaveLength(1);
    } finally {
      await store.close();
      fs.rmSync(homeDir, { recursive: true, force: true });
    }
